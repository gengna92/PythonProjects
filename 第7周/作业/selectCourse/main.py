...
if __name__ == "__main__":
    ...
    if flag == 1:
        stu = STU(name)
        stu_menu = ['stu.show()','coursename = input("请输入您要选择的课程：").strip()\nstu.makeCourse(coursename)',
                    'stu.haveChoose()','print("退出登入")\nflag = 0']
        while flag == 1:
            PUBLIC.showMenu(1)
            try:
    ...
    if flag == 3:
        ...
        teacher_menu = ['teacher.show()','teacher.selectClass()','clsname = input("请输入您要查找的班级：").strip()\n'
                        'teacher.selectStu(clsname)','print("退出登入")\nflag = 0']
        while flag == 3:
            teacher.showMenu(3)
            try:
                num = int(input("请输入您的选择：").strip())
