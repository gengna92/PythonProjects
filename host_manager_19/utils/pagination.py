...
class Pagination:
    ...
    def __init__(self, page, all_count, query_param=QueryDict(mutable=True), per_num=10, max_show=11):
        """
        
        :param page: 当前页码数
        :param all_count:  总数据条数
        :param query_param:  查询的参数
        :param per_num:  每页数据量
    ...
    def page_html(self):
        ...
        if self.page == 1:
            ...
            self.query_param['page'] = self.page - 1
            page_list.append('<li><a href="?{}">&laquo;</a></li>'.format(self.query_param.urlencode()))
        
        for i in range(self.page_start, self.page_end + 1):
            self.query_param['page'] = i
            if self.page == i:
                page_list.append('<li class="active"><a href="?{}">{}</a></li>'.format(self.query_param.urlencode(), i))
            else:
                page_list.append('<li><a href="?{}">{}</a></li>'.format(self.query_param.urlencode(), i))
        
        if self.page == self.page_count:
            page_list.append('<li class="disabled"><a>&raquo;</a></li>')
        else:
            self.query_param['page'] = self.page + 1
            page_list.append('<li><a href="?{}">&raquo;</a></li>'.format(self.query_param.urlencode()))
